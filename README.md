...
- Добавлены новые эндпоинты в API
  1. Добавление документа в коллекцию (POST /api/collections/<collection_id>/upload)
  2. Создание коллекции (POST /api/collections)
- Удалены ненужные сущности из data.py
  1. class Documents
  2. class Collections
  3. database.get_top_words_for_document()
  4. database.get_document_frequency()
  5. database.get_documents_count()
  6. database.insert_document()
- recalculate_idf() стал методом DataBase
- Все переменные среды импортируются в новом файле config.py, впоследствии импортируясь оттуда в нужные места.
