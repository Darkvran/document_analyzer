    ...
    if request.method == "POST":
        ...
        if existing_user:
            if (
                ...
                ).hexdigest()  # Проверка верности пароля с шифрованием
                == existing_user["h_password"]
            ):
                user = User(
                    existing_user["_id"],
                    existing_user["username"],
                    existing_user["collection_ids"],
                )
                login_user(user)
                return redirect(url_for("index_bp.index"))
        else:
...
def register():
    ...
    if request.method == "POST":
        ...
        ) or database.users.find_one({"email": request.form["email"]})
        if existing_user:
        ...
        user = {
            ...
            "h_password": hashlib.sha256(
                request.form["password"].encode()
            ).hexdigest(),  # Шифрование пароля
            "collection_ids": [],
        }
